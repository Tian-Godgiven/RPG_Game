...
type Package = Record<string,Item|Money>
...
function package_list(packages:Package){
...
function change_package_table(type:string){
...
//把货币对象和物品对象拆开来调用
//获取到货币时，货币数量上升
export function getMoney(money:Money|null,number:number){
    //没有指定货币对象时，获取默认的货币对象
	if(money == null){
