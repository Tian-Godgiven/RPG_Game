...
import { changeScene } from "../scene/scene"
...
//下一页
function nextPage(returned?:boolean){
	...
	switch (page_counter) {
	    ...
	    case 2:
	    	YOU["race"] = All_entity[$("#newgame_race_select").val()]["name"]
	      	printLog("你的种族是"+All_entity[$("#newgame_race_select").val()]["name"]+"。");
	      	break;  
	    case 3:
	    	YOU["gender"] = $('input:radio[name=gender]').filter(':checked').val()
	    	printLog("你是"+$('input:radio[name=gender]').filter(':checked').val()+"。")
...
$("#newgame_character_return").on("mousedown",function(){
	...
	page_counter-=1;
	return_counter+=1
	let log = return_logs[Math.floor(Math.random() * return_logs.length)]
	if(return_counter==9){
